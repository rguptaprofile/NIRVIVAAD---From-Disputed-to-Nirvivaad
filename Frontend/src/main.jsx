...
function GisParcelViewer({ parcelData, onApiKeyUpdate }) {
  const [activeMode, setActiveMode] = useState('mapbox'); // 'mapbox' | 'blueprint'
  const [activeStyle, setActiveStyle] = useState('satellite'); // 'streets' | 'satellite'
  const [apiKeyInput, setApiKeyInput] = useState('pk.eyJ1IjoicnJhai1paXRwIiwiYSI6ImNtdTZiZm40cjA5M2gyeHNsNjhvenQ1dGsifQ.NW68R9t_5yV99rAnhb0ezA');
  const [customKeyStatus, setCustomKeyStatus] = useState('Mapbox GL JS Active (Verified)');
  const [mapError, setMapError] = useState(null);
  const [activeLocation, setActiveLocation] = useState('cadastral'); // 'cadastral' | 'sample'
  ...
  function handleKeyApply() {
    if (apiKeyInput.trim().length >= 10) {
      setCustomKeyStatus('Mapbox Token Applied & Connected');
  ...
  function initMapbox() {
    ...
    try {
      setMapError(null);
      const token = apiKeyInput.trim() || 'pk.eyJ1IjoicnJhai1paXRwIiwiYSI6ImNtdTZiZm40cjA5M2gyeHNsNjhvenQ1dGsifQ.NW68R9t_5yV99rAnhb0ezA';
      ...
      const lng = activeLocation === 'sample' ? -74.0060 : (centroid.longitude || 85.3910);
      const lat = activeLocation === 'sample' ? 40.7128 : (centroid.latitude || 26.1197);
      const zoomLevel = activeLocation === 'sample' ? 12 : 16;

      const styleUrl = activeStyle === 'satellite'
      ...
      map.on('load', () => {
        // 1. Cadastral Boundary Polygon GeoJSON Source
        const dLng = 0.0012;
        const dLat = 0.0008;
        const polyCoords = [
          [lng - dLng, lat - dLat],
        ...
        map.addSource('cadastral-parcel-source', {
          type: 'geojson',
          data: {
                ...
                properties: {
                  description: `
                    <div style="font-family:sans-serif;padding:4px;">
                      <b style="color:#1B4332;font-size:13px;">Cadastral Plot (WGS84)</b><br/>
                      <b>Bhu-Aadhaar ULPIN:</b> <code style="color:#156B3A;font-weight:700;">${ulpin}</code><br/>
                      <b>Khasra:</b> ${parcelData.khasra_no || '214/2'} · <b>Khata:</b> ${parcelData.khata_no || '47'}<br/>
                      <b>Survey Area:</b> ${areaAc} Acres (${areaM2} m²)<br/>
                      <b>Status:</b> <span style="color:#156B3A;font-weight:700;">✓ Nirvivaad (Title Clear)</span>
                    </div>
                  `
                },
                geometry: {
        ...
        map.addLayer({
          id: 'cadastral-fill',
          type: 'fill',
          source: 'cadastral-parcel-source',
          paint: {
            'fill-color': '#2E7D32',
            'fill-opacity': 0.38
          }
        ...
        // 2. Corner Vertex Pins & Sample GeoJSON points (Matching user's circle styling #B71C1C)
        map.addSource('my-geojson-features', {
          type: 'geojson',
          data: {
            type: 'FeatureCollection',
            features: [
              {
                type: 'Feature',
                properties: {
                  description: `<div style="font-family:sans-serif;"><b>Cadastral Centroid</b><br/>GPS: ${lat.toFixed(4)}°N, ${lng.toFixed(4)}°E<br/>ULPIN: ${ulpin}</div>`
                },
                geometry: { type: 'Point', coordinates: [lng, lat] }
              },
              {
                type: 'Feature',
                properties: { description: '<b>Corner Vertex P1 (NW)</b>' },
                geometry: { type: 'Point', coordinates: polyCoords[3] }
              },
              {
                type: 'Feature',
                properties: { description: '<b>Corner Vertex P2 (NE)</b>' },
                geometry: { type: 'Point', coordinates: polyCoords[2] }
              },
              {
                type: 'Feature',
                properties: { description: '<b>Corner Vertex P3 (SE)</b>' },
                geometry: { type: 'Point', coordinates: polyCoords[1] }
              },
              {
                type: 'Feature',
                properties: { description: '<b>Corner Vertex P4 (SW)</b>' },
                geometry: { type: 'Point', coordinates: polyCoords[0] }
              }
            ]
          }
        });
        ...
        map.on('click', 'geojson-layer', (e) => {
          ...
          new window.mapboxgl.Popup({ offset: 10 }).setLngLat(coords).setHTML(desc).addTo(map);
        });

        map.on('click', 'cadastral-fill', (e) => {
          if (!e.features || !e.features[0]) return;
          const desc = e.features[0].properties.description;
          new window.mapboxgl.Popup({ offset: 10 }).setLngLat(e.lngLat).setHTML(desc).addTo(map);
        });

        map.on('mouseenter', 'geojson-layer', () => { map.getCanvas().style.cursor = 'pointer'; });
        map.on('mouseleave', 'geojson-layer', () => { map.getCanvas().style.cursor = ''; });
      });
  ...
  }, [activeMode, activeStyle, activeLocation]);

  function flyToLocation(loc) {
    setActiveLocation(loc);
    if (mapInstanceRef.current) {
      const targetLng = loc === 'sample' ? -74.0060 : (centroid.longitude || 85.3910);
      const targetLat = loc === 'sample' ? 40.7128 : (centroid.latitude || 26.1197);
      const targetZoom = loc === 'sample' ? 12 : 16;
  ...
  return (
    <div className="gis-viewer-panel">
      <div className="gis-top-bar">
        ...
        <div className="gis-api-bar">
          <input
            ...
            placeholder="Enter Mapbox Access Token (pk....)"
            value={apiKeyInput}
      ...
      <div className="gis-content-grid">
        {/* Interactive Vector GIS Canvas / Mapbox GL Container */}
        <div className="gis-map-viewport" style={{ position: 'relative', minHeight: 310, background: '#111' }}>
          {/* Top Layer & Mode Switcher */}
          <div className="gis-layer-toggle" style={{ zIndex: 10 }}>
            <button
              type="button"
              className={activeMode === 'mapbox' && activeStyle === 'satellite' ? 'active' : ''}
              onClick={() => { setActiveMode('mapbox'); setActiveStyle('satellite'); }}
            >
              🛰️ Mapbox Satellite
            </button>
            <button
              type="button"
              className={activeMode === 'mapbox' && activeStyle === 'streets' ? 'active' : ''}
              onClick={() => { setActiveMode('mapbox'); setActiveStyle('streets'); }}
            >
              🗺️ Mapbox Streets
            </button>
            <button
              type="button"
              className={activeMode === 'blueprint' ? 'active' : ''}
          ...
          {activeMode === 'mapbox' && (
            <div style={{ position: 'absolute', bottom: 10, left: 10, zIndex: 10, display: 'flex', gap: 6 }}>
              <button
                type="button"
                className={`btn btn-sm ${activeLocation === 'cadastral' ? 'btn-primary' : 'btn-ghost'}`}
                style={{ fontSize: 11, padding: '3px 8px', background: activeLocation === 'cadastral' ? '#183C2F' : 'rgba(255,255,255,0.9)', color: activeLocation === 'cadastral' ? '#FFF' : '#183C2F' }}
                onClick={() => flyToLocation('cadastral')}
              >
                📍 Cadastral Parcel (India)
              </button>
              <button
                type="button"
                className={`btn btn-sm ${activeLocation === 'sample' ? 'btn-primary' : 'btn-ghost'}`}
                style={{ fontSize: 11, padding: '3px 8px', background: activeLocation === 'sample' ? '#183C2F' : 'rgba(255,255,255,0.9)', color: activeLocation === 'sample' ? '#FFF' : '#183C2F' }}
                onClick={() => flyToLocation('sample')}
              >
                🌐 GeoJSON Sample (NYC)
              </button>
            </div>
          ...
          {/* Mode 1: Real Mapbox GL JS Container */}
          {activeMode === 'mapbox' ? (
            <div
              ref={mapContainerRef}
              style={{
            ...
            >
              {!window.mapboxgl && (
                <div style={{ padding: 20, color: '#FFF', textAlign: 'center', paddingTop: 90 }}>
                  <p>Loading Mapbox GL JS engine from CDN...</p>
                  <button className="btn btn-ghost btn-sm" style={{ color: '#FFF', borderColor: '#FFF' }} onClick={() => setActiveMode('blueprint')}>
                    Switch to Cadastral Blueprint
                  </button>
                </div>
              )}
            </div>
          ) : (
            /* Mode 2: High-Fidelity Cadastral Blueprint SVG Schematic */
            <svg width="100%" height="310" viewBox="0 0 400 310" style={{ display: 'block' }}>
              <defs>
                <pattern id="gisGrid" width="30" height="30" patternUnits="userSpaceOnUse">
                  <path d="M 30 0 L 0 0 0 30" fill="none" stroke="#CFD8D1" strokeWidth="0.6" />
                </pattern>
              </defs>
              <rect width="100%" height="100%" fill="#F4F8F5" />
              <rect width="100%" height="100%" fill="url(#gisGrid)" />

              {/* Adjacent Northern Plot */}
              <rect x="110" y="25" width="180" height="45" fill="#DEE7E1" stroke="#8DA896" strokeWidth="1" strokeDasharray="3 3" />
              <text x="200" y="52" textAnchor="middle" fontSize="10" fill="#576D5F">
                North: Plot {chauhaddi.north?.plot || '214/1'} (Private Raiyat)
              </text>

              {/* Adjacent Southern PWD Road Feature */}
              <rect x="80" y="225" width="240" height="35" fill="#EAE2CE" stroke="#BAA57E" strokeWidth="1" />
              <text x="200" y="247" textAnchor="middle" fontSize="10.5" fontWeight="600" fill="#7A6843">
                South: {chauhaddi.south?.plot || 'Sarkari Sadak (PWD Right-of-Way)'}
              </text>

              {/* Target Cadastral Plot Boundary Polygon */}
              <polygon
                points="130,95 270,95 270,205 130,205"
                fill="rgba(63, 107, 74, 0.22)"
                stroke="#2E7D32"
                strokeWidth="2.5"
              />

              {/* Corner Vertex Pins */}
              <circle cx="130" cy="95" r="4" fill="#B71C1C" stroke="#FFF" strokeWidth="1.5" />
              <text x="115" y="90" fontSize="9" fontWeight="700" fill="#B71C1C">P1</text>

              <circle cx="270" cy="95" r="4" fill="#B71C1C" stroke="#FFF" strokeWidth="1.5" />
              <text x="278" y="90" fontSize="9" fontWeight="700" fill="#B71C1C">P2</text>

              <circle cx="270" cy="205" r="4" fill="#B71C1C" stroke="#FFF" strokeWidth="1.5" />
              <text x="278" y="216" fontSize="9" fontWeight="700" fill="#B71C1C">P3</text>

              <circle cx="130" cy="205" r="4" fill="#B71C1C" stroke="#FFF" strokeWidth="1.5" />
              <text x="115" y="216" fontSize="9" fontWeight="700" fill="#B71C1C">P4</text>

              {/* Centroid Marker with Radar Glow */}
              <circle cx="200" cy="150" r="14" fill="none" stroke="#2E7D32" strokeWidth="1" className="radar-pulse" />
              <circle cx="200" cy="150" r="4.5" fill="#1B4332" stroke="#FFF" strokeWidth="1.5" />
              <text x="200" y="172" textAnchor="middle" fontSize="10" fontWeight="700" fill="#1B4332">
                Survey Centroid (ULPIN: {ulpin})
              </text>
            </svg>
          )}
        </div>
        ...
        <div className="gis-metrics-card">
          ...
          <div className="gis-stat-row">
            ...
            <b style={{ color: '#156B3A' }}>● GeoJSON Vector Active</b>
          </div>

          <table className="pins-table" style={{ marginTop: 10 }}>
            <thead>
              <tr>
                <th>Vertex Pin</th>
                <th>Latitude</th>
