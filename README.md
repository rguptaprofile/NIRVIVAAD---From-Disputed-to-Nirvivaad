...
2. Create `Backend/.env` with `MONGODB_URI=mongodb://localhost:27017`, a unique `JWT_SECRET_KEY`, and `ALLOWED_ORIGINS=[https://nirvivaad.vercel.app/]`. Do not commit this file.
...
- **Vercel:** import the repository with root directory unchanged. `vercel.json` builds `Frontend` and deploys only `Frontend/dist`. In **Settings → Environment Variables**, set `VITE_API_BASE_URL=https://nirvivaad-from-disputed-to-nirvivaad-5.onrender.com/api/v1` for Production, Preview, and Development; redeploy after saving it. The built-in fallback uses the same URL so the deployed app never tries `localhost`.
- **Render:** create a Blueprint from this repository. For an existing manually-created service, leave Root Directory blank, use build command `python -m pip install --upgrade pip && python -m pip install --prefer-binary -r requirements.txt`, and start command `uvicorn render_start:app --host 0.0.0.0 --port $PORT`. It needs an Atlas `MONGODB_URI`, a strong `JWT_SECRET_KEY`, and `ALLOWED_ORIGINS` set to the exact Vercel URL (without a trailing `/`). Set `OPENAI_API_KEY` only in Render—not in Vercel or frontend files. To create the first admin, set `BOOTSTRAP_ADMIN_EMAIL` and `BOOTSTRAP_ADMIN_PASSWORD` once; then remove the password setting.
